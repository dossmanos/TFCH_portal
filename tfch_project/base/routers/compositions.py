...
async def get_compositions(compositions: List[Composition] = Depends(adapters.retrieve_compositions),) -> FastCompositions:
    return FastCompositions.from_qs(compositions)

@router.get("/{composition_id}", response_model=FastComposition)
async def get_composition(composition: Composition= Depends(adapters.retrieve_composition)) -> FastComposition:
...
async def create_composition(set_polish_name:str, set_english_name:str ) -> dict:
    try:
        Composition.objects.create(
            polish_name = set_polish_name,
            english_name = set_english_name
        )
        return {'Server info':f'New composition {set_polish_name} added succesfully to database' }
    except:
        return {'Server info':'Error during adding composition to database'}

@router.put("/{composition_id}")
async def update_composition(new_polish_name: str, new_english_name: str, composition: Composition= Depends(adapters.retrieve_composition)) -> dict | FastComposition:
    try:
        ...
        return {'Server info':f'Error during updating composition no.{composition_to_update.id}'}

@router.delete("/{composition_id}")
async def delete_composition(composition: Composition= Depends(adapters.retrieve_composition)) -> dict:
    try:
